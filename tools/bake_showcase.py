...
# The bench and container fixtures. The website's board already hides these
# rows (robot-brawl/index.html, FIXTURES / FIXTURE_OWNERS) because
# "SecondEnlistee owned by Rival" reads to a stranger as a game nobody plays.
# The champions page was still free to NAME one as the machine a champion
# beat -- SUPER baked as "beat ThirdEnlistee (Challenger)" on 2026-08-21,
# which undercuts the exact impression the board filter exists to protect.
# Same list, kept deliberately narrow and exact-match, so a real player who
# names a robot "Grinder" is never quietly demoted.
FIXTURE_ROBOTS = {"secondenlistee", "thirdenlistee", "builtinthecloud"}
FIXTURE_OWNERS = {"containertest", "rival", "challenger"}


def is_fixture(name, owner):
    n = (name or "").strip().lower()
    o = (owner or "").strip().lower()
    return (n in FIXTURE_ROBOTS or o in FIXTURE_OWNERS
            or n.startswith("benchbot-")
            or (o and n and o == n + " pilot"))
...
def ranked_wins(matches):
    ...
    def gain(m):
        try:
            ...
            return -1e9
    # THREE tiers, not two: a real player's design (2) beats a bench fixture
    # (1) beats a house machine (0). Where only a lesser one exists we still
    # show it rather than leaving the class blank.
    def tier(m):
        if is_house(m.get("opponentOwner")):
            return 0
        if is_fixture(m.get("opponentName"), m.get("opponentOwner")):
            return 1
        return 2

    wins.sort(key=lambda m: (tier(m), gain(m), m.get("completedAt") or ""),
              reverse=True)
    return wins
